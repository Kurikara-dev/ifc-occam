...
- **CI covers the synthetic-fixture suite only, on Windows.** GitHub Actions runs the test suite on Windows; the real-model integration tests skip there — the models they need are not in the repository, so they run only on machines that have the data. A Linux CI job was tried and dropped: the manylinux `ifcopenshell` build segfaulted in the geometry path, which is consistent with this project's Windows-only support statement. Tagged releases (`v*`) build a Windows binary automatically; check [Releases](https://github.com/Kurikara-dev/ifc-occam/releases) for what has actually been published.
