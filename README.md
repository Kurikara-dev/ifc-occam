...
- **Text mode leaves a small residue.** It keeps a few unreferenced records that the full-open path removes — measured at +0.24% of records on a 380,000-record model, but that number predates the annotation-pinning fix below and has not been re-measured since. Layer assignments and style records no longer pin the deleted elements' geometry (they used to, which on layer-heavy models kept nearly all of it); a `IfcPresentationLayerWithStyle` assignment is the remaining known case that still does. Surviving elements and geometry match the full-open path exactly; the record count does not.
